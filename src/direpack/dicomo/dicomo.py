...
class dicomo(_BaseComposition, BaseEstimator):
    ...
    def __init__(self, est="arithmetic", mode="mom", center="mean"):
        ...
        if not (self.mode in self.limo):
            raise (
                MyException(
                    "Only models allowed are: 'mom','var','skew','kurt','com','cov','cos','cok','corr','continuum','M3','mdd','mdc','ballcov'"
                )
            )
        if not (self.est in self.liest):
            raise (
                MyException(
                    'Only estimator classes allowed are: "arithmetic", "distance", "sign", "entropy"'
                )
            )
        if not (self.center in self.licenter):
            raise (
                MyException(
                    'Only centring classes allowed are: "mean", "median"'
                )
            )
        ...
        if self.mode == "ballcov":
            print(
                "To use the Ballcov functionality, uncomment the import statement and comment this line"
            )
    ...
    def fit(self, x, **kwargs):
        ...
        if "soft_trim" not in kwargs:
            soft_trim = True
        else:
            soft_trim = kwargs.get("soft_trim")
        ...
        if self.est == "arithmetic":
            ...
            if mode != "com":
                ...
                #                    xvar = srs.mad(x)**2
                xmom = trim_mom(
                    x, x, locest, order, trimming, option, biascorr
                )
                self.x_moment_ = xmom
            ...
            if mode != "mom":
                ...
                if n1 != n:
                    raise (
                        MyException("Please feed x and y data of equal length")
                    )
                if len(y.shape) == 1:
                    y = np.array(y).reshape((n, 1))

                como = trim_mom(
                    x, y, locest, order, trimming, option, biascorr
                )
                ...
                if self.mode in ("cok", "cos"):
                    ...
                    if (
                        self.mode == "cok"
                    ) and biascorr:  # biascorr is only exact if standardized
                        como -= como / (ntrim**2)
                        como -= (
                            3
                            * (ntrim - 1) ** 2.0
                ...
                if mode in ["corr", "continuum"]:
                    ...
                    #                        yvar = srs.mad(y)
                    ymom = trim_mom(
                        y, y, locest, order, trimming, option, biascorr
                    )
