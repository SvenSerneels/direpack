...
class ppdire(_BaseComposition, BaseEstimator, TransformerMixin, RegressorMixin):
    ...
    def fit(self, X, *args, **kwargs):
        ...
        if (
            self.projection_index == dicomo
            and self.pi_arguments["mode"] == "kurt"
            and self.whiten_data == False
        ):
        ...
        if self.optimizer == "grid":
            ...
            nangle = np.linspace(
                anglestart, anglestop, self.optimizer_options["ndir"], endpoint=False
            )
        ...
        for i in range(0, h):
            ...
            if self.optimizer == "grid":
                if p == 2:
                    wi, maximo = gridplane(E, self.most, pi_arguments=opt_args)
                    ...
                    meas = [self.most.fit(E[:, k], **opt_args) for k in np.arange(0, p)]
                    if self.square_pi:
                        meas = np.square(meas)
                    ...
                    for j in np.arange(2, p):
                        projmat = np.matrix(
                            [
                                np.array(Zopt[:, 0]).reshape(-1),
                                np.array(Z[:, j]).reshape(-1),
                            ]
                        ).T
                        wi, maximo = gridplane(projmat, self.most, opt_args)
                        Zopt = Zopt * float(wi[0]) + Z[:, j] * float(wi[1])
                        afin[0 : (j + 1)] = afin[0 : (j + 1)] * float(wi[0])
                        afin[j] = float(wi[1])

                    tj = Z * afin
                    objf = self.most.fit(tj, **{**fit_arguments, **opt_args})
                    if self.square_pi:
                        objf *= objf
                    ...
                    while (ii < self.optimizer_options["maxiter"] + 1) and (
                        abs(objfold - objf) / abs(objf) > 1e-4
                    ):
                        ...
                        objfold = copy.deepcopy(objf)
                        objf = self.most.fit(tj, q=afin, **opt_args)
                        if self.square_pi:
                            objf *= objf
            ...
            if flag != "one-block":
                criteval = self.most.fit(E * wi0, **opt_args)
                if self.square_pi:
                    criteval *= criteval
        ...
        if flag != "one-block":
            ...
            if not (scale == "None"):
                if self.center == "mean":
                    ...
                    b0 = np.median(
                        np.array(
                            ys.astype("float64") - np.matmul(Xs.astype("float64"), bi)
                        )
                    )
            else:
                b0 = intercept
    ...
    def predict(self, Xn):
        ...
        return np.array(
            np.matmul(Xn, self.coef_[:, h - 1]) + self.intercept_
        ).T.reshape(-1)
