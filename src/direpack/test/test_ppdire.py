...
class Testppdire(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.datav = self.data.values[:, 2:8].astype("float64")
    ...
    def test_pca(self):
        ...
        np.testing.assert_almost_equal(
            np.abs(pppca.x_loadings_), np.abs(skpca.components_.T), decimal=3
        )
    ...
    def test_pls(self):
        ...
        np.testing.assert_almost_equal(
            np.abs(
                np.matmul(self.Xs, skpls.coef_.reshape(-1)) * np.std(self.y)
                + np.mean(self.y)
            ),
            np.abs(pppls.fitted_.ravel()),
            decimal=3,
        )
