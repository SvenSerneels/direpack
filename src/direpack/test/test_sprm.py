...
class Testsprm(unittest.TestCase):
    ...
    def test_sprm(self):
        ...
        np.testing.assert_almost_equal(
            np.linalg.norm(res_sprm.weightnewx(self.x0[2666:])),
            test_ans,
            decimal=4,
        )
    ...
    def test_rm(self):
        ...
        np.testing.assert_almost_equal(
            np.linalg.norm(res_rm.predict(self.x0[2666:])), test_ans, decimal=4
        )

    def test_snipls(self):
        """Test the functioning of the snipls object"""
        res_snipls = snipls(n_components=4, eta=0.5)
        res_snipls.fit(self.x0[:2666], self.y0[:2666])
        test_ans = 38.6183244001568
        np.testing.assert_almost_equal(
            np.linalg.norm(res_snipls.predict(self.x0[2666:])),
            test_ans,
            decimal=4,
        )
        self.x[0, 0] = np.nan
